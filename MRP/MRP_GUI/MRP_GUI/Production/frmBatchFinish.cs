...
namespace MRP_GUI
{
    public partial class frmBatchFinish : System.Windows.Forms.Form
    {

        User CurrentUser = new User();


        public frmBatchFinish(User objUser)
        {
            InitializeComponent();
            CurrentUser = objUser;
        }
        ...
        private void Load_Batch_List()
        {
            try
            {
                ...
                DataTable dt = objBatch_DL.Get_Batch_ToFinish(CurrentUser.UserID,(int)Batch.Status.To_Finish);
                ...
                dgvBatcList_CellClick(sender, e);
            }
            catch (Exception)
            {
                ...
                if (theResult == System.Windows.Forms.DialogResult.Yes)
                {
                    ...
                    else
                    {
                        if (objQCReport_DL.Get_IsManufacturingQCPass(objBatch.BatchID, (int)QCReport.ReportStatus.Accept).Rows.Count > 0)
                        {
                            ...
                            //{
                            objBatch_DL.Update_Finish(objBatch);
                            objBatch_DL.Update_BatchCost(objBatch);
                                Load_Batch_List();
        ...
        private void dgvBatchLabourDetails_CellContentClick(object sender, DataGridViewCellEventArgs e)
        {
