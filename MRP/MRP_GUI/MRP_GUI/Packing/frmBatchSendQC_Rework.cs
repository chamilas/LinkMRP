...
namespace MRP_GUI
{
    public partial class frmBatchSendQC_Rework : System.Windows.Forms.Form
    {

        User CurrentUser = new User();

        public frmBatchSendQC_Rework(User objUser)
        {
            InitializeComponent();
            CurrentUser = objUser;
        }
        ...
        private void Load_Batch_List()
        {
            try
            {
                ...
                DataTable dt = objBatch_DL.Get_DataView_ReworkBatchList((int)Batch.Status.Finished, "Rework", "Repack");
        ...
        private void btnSendQC_Click(object sender, EventArgs e)
        {
            if (DataValidation.IsNumericNumber(txtSampleQty.Text))
            {
                try
                {
                    ...
                    long x = objQCReport_DL.Add_Packing_Rework(objQCReport);

                    if (x > 0)
                    {
                        MessageBox.Show(this, "Successfully Send QC", "Successful", MessageBoxButtons.OK);

                        objBatch_DL.Update_QCStatus_ForReworkBatches(dgvBatcList.CurrentRow.Cells["BatchNo"].Value.ToString(), CurrentUser.EmployeeID, Batch.Status.QC);
                    }
        ...
        private void Load_ReportList()
        {
            try
            {
                DataTable dtQc = objQCReport_DL.GetView_PackingReworkQC(objBatch.BatchID);
