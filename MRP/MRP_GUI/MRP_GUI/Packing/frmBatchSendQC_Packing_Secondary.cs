...
namespace MRP_GUI
{
    public partial class frmBatchSendQC_Packing_Secondary : System.Windows.Forms.Form
    {

        User CurrentUser = new User();

        public frmBatchSendQC_Packing_Secondary(User objUser)
        {
            InitializeComponent();
            CurrentUser = objUser;
        }
        ...
        private void btnSendQC_Click(object sender, EventArgs e)
        {
            if (DataValidation.IsNumericNumber(txtSampleQty.Text))
            {
                try
                {
                    ...
                    objQCReport_DL.Add_Packing_Secondary(objQCReport);
        ...
        private void Load_ReportList()
        {
            try
            {
               DataTable dtQc= objQCReport_DL.GetView_PackingQC_Sec(objBatch.BatchID);
