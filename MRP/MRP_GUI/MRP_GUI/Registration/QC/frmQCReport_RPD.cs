...
namespace MRP_GUI
{
    public partial class frmQCReport_RPD : System.Windows.Forms.Form
    {
        public frmQCReport_RPD(User objuser)
        {
            CurrentUser = objuser;
            InitializeComponent();
        }
        ...
        private void frmQCReport_Load(object sender, EventArgs e)
        {
            try
            {
                ...
                cmbTestBy.DataSource = objEmployee_DL.Get_By_DepType("LAB", true);
                    ...
                    cmbParameter.DataSource = objTestParameter_DL.GetView("RPD");
                     ...
                     if (rbAccept.Checked)
                        {
                            ...
                            objQCReport_DL.Update_Production(objQCReport, Convert.ToInt32(Batch.Status.QCPass), objBatch.BatchID, CurrentUser.EmployeeID);

                        }
                        else if (rbReject.Checked)
                        {
                            objQCReport.Status = Convert.ToInt32(QCReport.ReportStatus.Reject);
                            objQCReport_DL.Update_Production(objQCReport, Convert.ToInt32(Batch.Status.QCFail), objBatch.BatchID, CurrentUser.EmployeeID);
