...
/****** Object:  StoredProcedure [dbo].[SPGET_PackingMaterialsForecast]    Script Date: 5/17/2014 10:13:19 PM ******/
SET ANSI_NULLS ON
GO
...
*/
		...
		SELECT tblSalesForecastDetails.ProductCode, tblSalesForecastDetails.Qty
		FROM tblSalesForecastDetails INNER JOIN tblSalesForecast ON tblSalesForecastDetails.SalesForecastID = tblSalesForecast.ID
		WHERE tblSalesForecast.Year=@prmYear AND tblSalesForecast.Month=@prmMonth AND State=1 AND 
		RevisionID=(SELECT max(tblSalesForecastDetails.RevisionID)
		FROM tblSalesForecastDetails INNER JOIN tblSalesForecast ON tblSalesForecastDetails.SalesForecastID = tblSalesForecast.ID
		WHERE tblSalesForecast.Year=@prmYear AND tblSalesForecast.Month=@prmMonth)
